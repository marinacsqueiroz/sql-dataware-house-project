...
#         logger.info("Schema and model creation successfully completed.")
#     else:
...
try:
    ...
    logger.info(f"Dataset analysis completed. Configurations saved to: {data_config_file_path}")
...
# try:
#     result_create_table = create_table(data_config_file_path, datasets_file_path, logger=logger)
    
#     logger.info("Table (or DDL/DML model) creation successfully completed.")

# except Exception as e:
#     logger.error(f"CRITICAL ERROR during table creation: {e}", exc_info=True)

logger.info("--- MAIN PROCESS CONCLUDED ---")
